...
import "../css/NoliktavuDarbinieksPreces.css";
import EditProductModal from "./EditProductModal";

function NoliktavuDarbinieksPreces() {
    const [precesData, setPrecesData] = useState([]);
    const [editingProduct, setEditingProduct] = useState(null);
    const [isEditModalOpen, setIsEditModalOpen] = useState(false);

    useEffect(() => {
        fetch("http://localhost/noliktava_php/selectPreces.php")
    ...
    const handleDelete = (id) => {
        fetch(`http://localhost/noliktava_php/deletePreces.php?id=${id}`, {
            method: "DELETE",
        })
            .then((response) => response.json())
            .then(() => {
    ...
    const handleProductUpdate = async (id, updatedProduct) => {
        try {
            const response = await fetch(`http://localhost/noliktava_php/selectPreces.php?id=${id}`);
...
export default NoliktavuDarbinieksPreces;
